...
def prepare_rolling_train(df, features_column, label_column, date_column, unique_datetime, testing_windows,
                          first_trade_date_index, max_rolling_window_index, current_index):
...
def prepare_rolling_test(df, features_column, label_column, date_column, unique_datetime, testing_windows,
                         fist_trade_date_index, current_index):
...
def prepare_trade_data(df, features_column, label_column, date_column, tic_column, unique_datetime, testing_windows,
                       fist_trade_date_index, current_index):
...
def train_svm(X_train, y_train):
    ...
    svm_regressor = GridSearchCV(estimator=svr, param_grid=param_grid_svm, cv=3, n_jobs=-1, scoring=scoring_method,
                                 verbose=0)
...
def append_return_table(df_predict, unique_datetime, y_trade_return, trade_tic, current_index):
    # old code (not working in new pandas version)
    ...
    # new code:
    new_row = pd.DataFrame([y_trade_return], columns=trade_tic)
    df_predict.loc[unique_datetime[current_index]][new_row.columns] = new_row.loc[0]
...
def run_4model(df, features_column, label_column, date_column, tic_column,
    ...
    for i in range(first_trade_date_index, len(unique_datetime)):
        try:
            ...
            y_trade_best = eval_table.model_predict_return.values[eval_table.model_eval == eval_table.model_eval.min()][
                0]
...
def calculate_portfolio_quarterly_return(quarterly_return, trade_date_plus1, long_dict):
    ...
    for i in range(len(trade_date_plus1) - 1):
        ...
        # map date and tic
        long_tic_return = quarterly_return[quarterly_return.index == trade_date_plus1[i + 1]][
            long_dict[trade_date_plus1[i]].index]
...
def long_only_strategy_daily(df_predict_return, daily_return, trade_month_plus1, top_quantile_threshold=0.75):
    ...
    for i in range(len(trade_month_plus1) - 1):
        ...
        long_tic_return_daily = \
            daily_return[
                (daily_return.index >= trade_month_plus1[i]) & (daily_return.index < trade_month_plus1[i + 1])][
